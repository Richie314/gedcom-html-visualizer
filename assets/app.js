...
import { GEDCStruct, g5ConfGEDC } from 'https://cdn.jsdelivr.net/gh/gedcom7code/js-gedcom@main/gedcstruct.js';

const getElement = (id) => document.getElementById(id);
const uploadZone = getElement('uploadZone');
const fileInput = getElement('fileInput');
const fileMessage = getElement('fileMessage');
...
function parseGedcom(text) {
  ...
  people.forEach((person) => {
    person.record.sub.filter((entry) => ['BIRT', 'DEAT', 'BAPM', 'CHR', 'BURI', 'RESI', 'OCCU', 'EDUC', 'EMIG', 'IMMI', 'NOTE'].includes(entry.tag))
      .forEach((entry, index) => events.set(`${person.id}:${entry.tag}:${index}`, {
        id: `${person.id}:${entry.tag}:${index}`, tag: entry.tag, record: entry, personId: person.id
      }));
  });
...
function labelSprite(lines, color = '#ffffff', background = 'rgba(20,30,52,.9)', compact = false) {
  ...
  context.fillStyle = background; context.roundRect(0, 0, width, canvas.height, 12); context.fill();
  ...
  } else context.fillText(labelLines[0], 15, compact ? 29 : 31);
  if (labelLines[1]) {
...
function showDetails(item) {
  ...
  if (item.kind === 'event') {
    ...
    detailPanel.innerHTML = `<div class="small text-primary fw-semibold mb-1">${escapeHtml(item.tag)} EVENT</div><h3 class="h5 mb-3">${escapeHtml(eventTitle)}</h3><div class="gedcom-rows">${recordRows(item.record)}</div>${linkedButtons(item)}`;
  } else if (item.kind === 'person') {
    const sexLabel = item.sex === 'F' ? 'WOMAN' : item.sex === 'M' ? 'MAN' : 'PERSON';
    detailPanel.innerHTML = `<div class="d-flex justify-content-between gap-3"><div><div class="small text-primary fw-semibold mb-1">${sexLabel} · ${item.id}</div><h3 class="h5 mb-2">${escapeHtml(item.givenName)} <em>${escapeHtml(item.surname)}</em></h3></div><div class="fs-3">◉</div></div><div class="gedcom-rows mt-3">${recordRows(item.record)}</div>${linkedButtons(item)}`;
  } else {
    detailPanel.innerHTML = `<div class="small text-warning-emphasis fw-semibold mb-1">MARRIAGE · ${item.id}</div><h3 class="h5 mb-3">${item.date ? `Married ${escapeHtml(item.date)}` : 'Marriage'}</h3><div class="gedcom-rows">${recordRows(item.record)}</div>${linkedButtons(item)}`;
  }
}
...
function generationPositions(graph) {
  ...
  levels.forEach((level, id) => { if (!grouped.has(level)) grouped.set(level, []); grouped.get(level).push(id); });
  graph.people.forEach((person, id) => {
    const level = levels.get(id), members = grouped.get(level), index = members.indexOf(id);
    person.position = new THREE.Vector3((index - (members.length - 1) / 2) * 4.2, -level * 4.6, (index % 2) * 1.2 - .6);
  });
  graph.connectors.forEach((connector) => {
...
function buildGraph(graph) {
  ...
  graph.connectors.forEach((connector) => {
    ...
    addNode(connector.position, color, connector.label, { ...connector, kind: 'connector' }, true);
  });
  getElement('peopleCount').textContent = graph.people.size;
  getElement('familyCount').textContent = graph.connectors.size;
  getElement('connectionCount').textContent = graph.links.length;
  getElement('generationCount').textContent = grouped.size || 0;
...
function addNode(position, color, label, data, connector = false) {
  const geometry = connector ? new THREE.OctahedronGeometry(.32, 0) : new THREE.SphereGeometry(.58, 20, 14);
  const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ color, roughness: .35, metalness: .1 }));
  mesh.position.copy(position); mesh.userData = data; scene.add(mesh); interactive.push(mesh);
  ...
  labelSpriteNode.position.copy(position).add(new THREE.Vector3(0, connector ? .55 : 1, 0)); scene.add(labelSpriteNode);
}

function initScene() {
  const canvas = getElement('scene');
  renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: false });
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); renderer.setClearColor(0x10182b);
  scene = new THREE.Scene(); camera = new THREE.PerspectiveCamera(50, 1, .1, 1000);
  camera.position.set(0, 0, 24); controls = new OrbitControls(camera, canvas); controls.enableDamping = true; controls.minDistance = 5; controls.maxDistance = 120;
  scene.add(new THREE.AmbientLight(0xffffff, 1.8)); const light = new THREE.DirectionalLight(0xffffff, 2.4); light.position.set(5, 10, 12); scene.add(light);
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    raycaster.setFromCamera(pointer, camera); showDetails(raycaster.intersectObjects(interactive)[0]?.object.userData);
  });
...
function loadFile(file) {
  if (!file) return;
  if (!/\.(ged|gedcom|txt)$/i.test(file.name)) { fileMessage.textContent = 'Please choose a .ged or .gedcom file.'; fileMessage.className = 'small mt-2 text-center text-danger'; return; }
  const reader = new FileReader();
  reader.onload = () => {
    try {
      const graph = parseGedcom(String(reader.result));
      fileMessage.textContent = `${file.name} loaded · ${graph.people.size} people found`;
      fileMessage.className = `small mt-2 text-center ${graph.errors.length ? 'text-warning' : 'text-success'}`;
      buildGraph(graph);
    } catch (error) {
      fileMessage.textContent = `Could not parse the GEDCOM file: ${error.message}`;
      fileMessage.className = 'small mt-2 text-center text-danger';
    }
  };
  reader.onerror = () => { fileMessage.textContent = 'The file could not be read.'; fileMessage.className = 'small mt-2 text-center text-danger'; };
  reader.readAsText(file);
}
getElement('chooseButton').addEventListener('click', (event) => { event.stopPropagation(); fileInput.click(); });
...
const target = event.target.closest('[data-inspect-id], [data-event-id]');
if (!target) return;
const item = target.dataset.eventId
  ? currentGraph.events.get(target.dataset.eventId)
  : currentGraph.people.get(target.dataset.inspectId) || currentGraph.connectors.get(target.dataset.inspectId);
showDetails(item ? { ...item, kind: target.dataset.eventId ? 'event' : item.kind || (currentGraph.people.has(item.id) ? 'person' : 'connector') } : null);
});
initScene();
